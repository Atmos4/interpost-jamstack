<script> 
    ...
    let scheme;
    const localStorageKey= "preferedColorScheme";

    function applyScheme() {
        document.querySelector("html").setAttribute("data-theme", scheme);
    }

    function schemeToLocalStorage() {
    ...
    onMount(async () => {
        ...
        if (typeof window.localStorage !== "undefined") {
            if (window.localStorage.getItem(localStorageKey) !== null) {
                scheme = window.localStorage.getItem(localStorageKey);
                applyScheme();
                return;
            }
        }
...
<div class="theme-switcher" on:click={switchTheme}>
    {#if scheme=="dark"}
